...
// WithFields adds multiple fields to the request logger
...
// WithError adds an error to the request logger and sets level to error
...
// SetLevel sets the log level for the final log entry
func (rl *RequestLogger) SetLevel(level slog.Level) *RequestLogger {
	rl.level = level
	return rl
}

// SetMessage sets the message for the final log entry
func (rl *RequestLogger) SetMessage(message string) *RequestLogger {
	rl.message = message
	return rl
}

// Log outputs the accumulated request data in a single log line
...
// NewRequestContext creates a new context with a request logger
...
// AddRequestField adds a field to the request logger in context
...
// AddRequestFields adds multiple fields to the request logger in context
...
// AddRequestError adds an error to the request logger in context
...
// LogRequest logs the accumulated request data
